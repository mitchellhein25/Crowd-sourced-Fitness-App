                ...
                ( <
                    ...
                    renderItem = {
                        ({ item }) => ( <
                            ...
                            key = { item.id }
                            onPress = {
                                () => navigation.navigate('Challenge Detail', { challenge: item, userId: state.id, user }) } >
                            <
                            Text style = { styles.description } > { item.description } < /Text> <
                            Text style = { styles.item } >
                            <
                            Text style = { styles.bold } > Type: & nbsp; < /Text> { challengeTypes.find((x) => x.id === item.type).item } <
                            /Text> { /* <Text style={styles.item}> */ } { /*    Created: */ } { /*    {item.date} */ } { /* </Text> */ } <
                            Text style = { styles.item } >
                            <
                            Text style = { styles.bold } > Goals: & nbsp; < /Text> {
                                item.goals.map((goal, index) => {
                                    return ( <
                                        Text key = { goal } > { goal } { index < item.goals.length - 1 ? ', ' : '' } <
                                        /Text>
                                    );
                                })
                            } <
                            ...
                            Text style = { styles.item } >
                            <
                            Text style = { styles.bold } > Tags: & nbsp; < /Text> {
                                item.tags.map((tag, index) => {
                                    return ( <
                                        Text key = { tag } > { tag } { index < item.tags.length - 1 ? ', ' : '' } <
                                        /Text>
                                    );
                                })
                            } <
                    ...
                    <
                    Button title = 'Show Active Challenges'
                    color = { primaryColor }
